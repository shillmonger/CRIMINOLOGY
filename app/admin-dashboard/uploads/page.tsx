...
export default function AdminContentUploadPage() {
  ...
  const handleTypeChange = (type: UploadType) => {
    ...
    if (type === "image") {
      setFormData({
        ...
        description:
          "This is a demo image preview shown for review purposes only.",
      ...
      setFormData({
        ...
        description:
          "This is a demo PDF preview shown for review purposes only.",
      ...
      setFormData({
        ...
        description:
          "This is a demo file preview shown for review purposes only.",
  ...
  const handleExternalUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Handle image URLs
        ...
        selectedType === "image" &&
        (url.match(/\.(jpeg|jpg|gif|png)$/) || url.includes("imgur.com"))
      ) {
    ...
    if (uploadType === "link") {
      ...
      try {
        ...
        if (selectedType === "pdf" && !externalUrl.match(/\.pdf($|\?)/i)) {
          return toast.error(
            "Please enter a valid PDF URL (must end with .pdf)"
          );
        }
        if (
          selectedType === "image" &&
          !externalUrl.match(/\.(jpeg|jpg|gif|png)($|\?)/i) &&
          !externalUrl.includes("imgur.com")
        ) {
          return toast.error(
            "Please enter a valid image URL (.jpg, .jpeg, .gif, .png or Imgur link)"
          );
        }
      } catch (error) {
          ...
          <div className="max-w-6xl mx-auto mb-5">
            <header className="mb-8">
              ...
              <p className="text-muted-foreground mt-1">
                Configure and publish your media assets
              </p>
            </header>
  ...
  <div className="space-y-8 w-full max-w-lg">
    ...
    <div>
      <label className="block text-[11px] font-black uppercase tracking-widest text-muted-foreground mb-4">
        Upload Source
      </label>
      <div className="flex justify-center space-x-3">
        <button
          ...
          className={`px-6 py-2.5 rounded-xl cursor-pointer text-xs font-bold uppercase tracking-tighter transition-all duration-200 border-2 ${
            uploadType === "file"
        ...
        <button
          ...
          className={`px-6 py-2.5 rounded-xl text-xs cursor-pointer font-bold uppercase tracking-tighter transition-all duration-200 border-2 ${
            uploadType === "link"
              ? "bg-foreground text-background border-foreground shadow-lg shadow-foreground/10"
              : "bg-transparent text-muted-foreground border-border hover:border-muted-foreground"
          }`}
        >
          {selectedType === "image" ? "Image Link" : selectedType === "video" ? "Video Link" : "PDF Link"}
        </button>
      </div>
    </div>

    {/* Content Type Selection (Only for File Upload) */}
    {uploadType === "file" && (
        ...
        <div className="flex justify-center space-x-2">
          {["image", "video", "pdf"].map((type) => (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-[10px] cursor-pointer font-black uppercase italic tracking-widest transition-all ${
                selectedType === type
    ...
    {uploadType === "file" ? (
      <div className="space-y-4">
        <label className="block text-[11px] font-black uppercase tracking-widest text-muted-foreground">
          Drop {selectedType} here
        </label>
        <div className="relative group cursor-pointer p-10 border-2 border-dashed border-border rounded-2xl hover:border-foreground/50 hover:bg-muted/30 transition-all duration-300">
          <div className="space-y-4 text-center">
            <div className="bg-foreground/5 w-16 h-16 rounded-full flex items-center justify-center mx-auto group-hover:scale-110 transition-transform">
                <svg className="h-8 w-8 text-foreground" stroke="currentColor" fill="none" viewBox="0 0 48 48">
            ...
            <div className="flex flex-col items-center gap-3">
              <label htmlFor="file-upload" className="relative cursor-pointer">
                <span className="bg-foreground text-background px-6 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest hover:bg-foreground/90 transition-colors shadow-xl">
                  Select {selectedType}
                </span>
                <input
                  id="file-upload"
            ...
            <p className="text-[9px] text-muted-foreground uppercase tracking-widest opacity-60">
              {selectedType === "image" ? "Max 10MB (PNG, JPG)" : selectedType === "video" ? "Max 50MB (MP4)" : "Max 20MB (PDF)"}
            </p>
          </div>
        </div>
        {fileName && (
          <div className="flex items-center justify-center gap-2 bg-foreground/5 p-2 rounded-lg border border-foreground/10">
      ...
      <div className="space-y-4 text-left">
        <label htmlFor="external-url" className="block text-[11px] font-black uppercase tracking-widest text-muted-foreground">
          {selectedType} URL
        </label>
        <div className="relative">
            ...
            className="w-full bg-muted/50 border-2 border-border rounded-xl p-4 text-sm font-medium focus:border-foreground focus:ring-0 outline-none transition-all placeholder:text-muted-foreground/40"
            />
        </div>
        <p className="text-[10px] text-muted-foreground italic font-medium">
          {selectedType === "image"
            ? "Supports direct links and Imgur"
            : selectedType === "video"
            ...
            : "Supports direct .pdf links"}
                ...
                >
                  ...
                  <button
                    ...
                    disabled={
                      isUploading ||
                      (uploadType === "file" ? !file : !externalUrl)
                    }
              ...
              <div className="space-y-6">
                      ...
                      {formData.tags.split(",").map(
                        (tag, i) =>
                          tag.trim() && (
                            <span
                              key={i}
                              className="text-[9px] font-black bg-muted text-muted-foreground px-2 py-1 rounded border border-border"
                            >
                ...
                <div className="bg-muted border border-border p-6 rounded-2xl">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Make sure your {selectedType} is high quality for better
                    engagement on the user dashboard.
                  </p>
                </div>
              </div>
            </div>
